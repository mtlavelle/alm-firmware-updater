...
;(function () {
  ...
  function logInfo(msg) {
    if (logContext) {
      let infos = document.getElementsByTagName('h4')
      // info.className = "info";
  ...
  document.addEventListener('DOMContentLoaded', event => {
            ...
            if (interfaces.length == 0) {
              ...
              async function connectToSelectedInterface() {
                ...
                if (filteredInterfaceList.length === 0) {
                  console.log(
                    'No interace with flash address 0x08000000 found.'
                  )
                  statusDisplay.textContent =
    ...
    downloadButton.addEventListener('click', async function (event) {
      ...
      if (device && firmwareFile != null) {
        ...
        await device
          ...
          .then(
            () => {
              logInfo('Module now updated and ready to use!')
              setLogContext(null)
    ...
    if (typeof navigator.usb !== 'undefined') {
      ...
      statusDisplay.textContent =
        'WebUSB not available, please use the Chrome browser.'
