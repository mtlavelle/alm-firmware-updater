...
;(function () {
  ...
  dfuse.parseMemoryDescriptor = function (desc) {
    ...
    while (
      ...
      while ((segmentMatch = segmentRegex.exec(contiguousSegmentMatch[0]))) {
        ...
        let sectorSize =
          parseInt(segmentMatch[2]) * sectorMultipliers[segmentMatch[3]]
  ...
  dfuse.Device.prototype.dfuseCommand = async function (command, param, len) {
    ...
    try {
      ...
      throw (
        'Error during special DfuSe command ' +
  ...
  dfuse.Device.prototype.do_download = async function (
    xfer_size,
    data,
    ...
    this.logInfo('Erasing current firmware...')
    ...
    this.logInfo('Copying firmware to module...')
  ...
  dfuse.Device.prototype.do_upload = async function (xfer_size, max_size) {
    ...
    this.logInfo(
      `Reading up to 0x${max_size.toString(
        16
      )} bytes starting at 0x${startAddress.toString(16)}`
    )
    ...
    return await dfu.Device.prototype.do_upload.call(
      this,
      xfer_size,
      max_size,
      2
